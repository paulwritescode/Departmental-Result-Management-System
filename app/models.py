...
class User(db.Model, UserMixin):
    ...
    student_enrollments = db.relationship(
        "StudentEnrollment", backref="student", lazy="dynamic"
    )
    lecturer_unit_assignment = db.relationship(
        "LecturerAssignment", backref="lecturer", lazy="dynamic"
    )
...
class Units(db.Model):
    ...
    student_enrollments = db.relationship(
        "EnrollmentUnits",
        backref="student_units",
        lazy="dynamic",
        overlaps="student_enrollments,student_units",
    )
    unit_lecturer = db.relationship(
        "LecturerAssignment", backref="lecs", lazy="dynamic"
    )
...
class Modules(db.Model):
    ...
    code = db.Column(db.Integer, unique=True, nullable=False)
    student_enrollments = db.relationship(
        "StudentEnrollment", backref="module", lazy="dynamic"
    )
...
class Marks(db.Model):
    __tablename__ = "marks"
    id = db.Column(db.Integer, primary_key=True)
    unitenrollment_id = db.Column(
        db.Integer, db.ForeignKey("enrollment_units.id"), nullable=False, unique=True
    )
